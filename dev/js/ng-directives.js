...
angular.module('app.directives', [])
	...
	.directive('photoGalleryEditBar', ['$rootScope', function($rootScope) {
		return {
			templateUrl: 'partials/templates/photo-gallery.edit-bar.html',
			restrict: 'A',
			require: '?^svElement',
			controller: function($scope) {
				if( ! $scope.Auth.isAuth) return;
				...
				$scope.deleteGallery = function(gallery, index) {
					if($scope.editBarMenu.confirm) {
						gallery.$delete().then(function() {
							$scope.galleries.photo.splice(index, 1);
							$scope.editBarMenu.toggleEdit();
							$scope.editBarMenu.confirm = false;
							$scope.editBarMenu.show = false;
						}, function() {
	...
	.directive('photoEntryEditBar', ['$rootScope', function($rootScope) {
		return {
			templateUrl: 'partials/templates/photo-entry.edit-bar.html',
			restrict: 'A',
			require: '?^svElement',
			controller: function($scope) {
				if( ! $scope.Auth.isAuth) return;
